...
class Normalize(TransformBase):
    ...
    def __init__(self, mean, std, inplace=False):
        ...
        # self.mean = [0.485, 0.456, 0.406, 0.485, 0.456, 0.406, 0.485, 0.456, 0.406, 0.485, 0.456, 0.406, 0.485, 0.456, 0.406, 0.485]        # HSI16
        # self.std = [0.229, 0.224, 0.225, 0.229, 0.224, 0.225, 0.229, 0.224, 0.225, 0.229, 0.224, 0.225, 0.229, 0.224, 0.225, 0.229]
        # self.inplace = inplace
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            # img_gray = cv.cvtColor(image, cv.COLOR_RGB2GRAY)            # lpp
            # return np.stack([img_gray, img_gray, img_gray], axis=2)     # lpp
            # return np.repeat(np.sum(img * self.color_weights, axis=2, keepdims=True).astype(np.uint8), 3, axis=2)
        return image            # max 255 min 0
