...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        if split is not None:
            ...
            ltr_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..')      # '/home/data/liupan/tracking/TransT-main/ltr/dataset/..'
            if split == 'train':
                ...
                file_path = os.path.join((self.root, 'list.txt'))
            else:
    ...
    def _read_bb_anno(self, seq_path):
        ...
        gt = pandas.read_csv(bb_anno_file, delimiter='\s+', header=None, dtype=np.float32, na_filter=False, low_memory=False).values     # delimiter=',',
        # GOT-10k
    ...
    def _read_target_visible(self, seq_path):
        ...
        visible_ratio = out_of_view.float() / 8
    ...
    def _get_frame_path(self, seq_path, frame_id):
        # clk HSI
        return os.path.join(seq_path, '{:04}.mat'.format(frame_id+1))    # frames start from 1          # lpp   .jpg
        # GOT10k
        # return os.path.join(seq_path, '{:08}.jpg'.format(frame_id+1))    # frames start from 1

    def _get_frame(self, seq_path, frame_id):
        # GOT10k
        # return self.image_loader(self._get_frame_path(seq_path, frame_id))        # (256, 512, 3)   lpp 
        # clk HSI
        image = sio.loadmat(self._get_frame_path(seq_path, frame_id))
        img = image['image']
        img = (img-img.min())/(img.max()-img.min())*255 
        ### PCA ###
        # w = img.shape[0]
        # h = img.shape[1]
        # dim_img = img.reshape(w*h,16)
        # pca = decomposition.PCA(n_components=15, copy=True, whiten=False)
        # dim_img = pca.fit_transform(dim_img)
        # img = dim_img.reshape(w,h,15)
        # img = (img-img.min())/(img.max()-img.min())*255 
        ### PCA ###
        return img
