...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        src_temp = src_temp.flatten(2).permute(2, 0, 1)     # torch.Size([256, 1, 256])
        pos_temp = pos_temp.flatten(2).permute(2, 0, 1)     # torch.Size([256, 1, 256])
        src_search = src_search.flatten(2).permute(2, 0, 1)     # torch.Size([1024, 1, 256])
        pos_search = pos_search.flatten(2).permute(2, 0, 1)     # torch.Size([1024, 1, 256])
        mask_temp = mask_temp.flatten(1)        # torch.Size([1, 256])
        mask_search = mask_search.flatten(1)    # torch.Size([1, 1024])
        ...
        memory_temp, memory_search = self.encoder(src1=src_temp, src2=src_search,
                                                  ...
                                                  pos_src1=pos_temp,
                                                  pos_src2=pos_search)      # torch.Size([256, 1, 256]) torch.Size([1024, 1, 256])
        hs = self.decoder(memory_search, memory_temp,
                          tgt_key_padding_mask=mask_search,
                          memory_key_padding_mask=mask_temp,
                          pos_enc=pos_temp, pos_dec=pos_search)     # torch.Size([1024, 1, 256])
...
class Decoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.norm is not None:
            output = self.norm(output)      # torch.Size([1024, 1, 256])
...
class Encoder(nn.Module):
    ...
    def forward(self, src1, src2,
        ...
        return output1, output2     # torch.Size([256, 1, 256])     torch.Size([1024, 1, 256])
...
class DecoderCFALayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, pos_dec),
                                   ...
                                   key_padding_mask=memory_key_padding_mask)[0]     # torch.Size([1024, 1, 256])
        tgt = tgt + self.dropout1(tgt2)     
        tgt = self.norm1(tgt)       # torch.Size([1024, 1, 256])
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))       # torch.Size([1024, 1, 256])
        tgt = tgt + self.dropout2(tgt2)     # torch.Size([1024, 1, 256])
        tgt = self.norm2(tgt)       # torch.Size([1024, 1, 256])
        return tgt
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
                     ...
                     pos_src1: Optional[Tensor] = None,
                     pos_src2: Optional[Tensor] = None):
        q1 = k1 = self.with_pos_embed(src1, pos_src1)       # q1:k1:shape:torch.Size([256, 1, 256])
        src12 = self.self_attn1(q1, k1, value=src1, attn_mask=src1_mask,
                               key_padding_mask=src1_key_padding_mask)[0]       # torch.Size([256, 1, 256])
        src1 = src1 + self.dropout11(src12)     
        src1 = self.norm11(src1)        # torch.Size([256, 1, 256])

        q2 = k2 = self.with_pos_embed(src2, pos_src2)       # torch.Size([1024, 1, 256])
        src22 = self.self_attn2(q2, k2, value=src2, attn_mask=src2_mask,
                               key_padding_mask=src2_key_padding_mask)[0]       # torch.Size([1024, 1, 256])
        src2 = src2 + self.dropout21(src22)     # torch.Size([1024, 1, 256])
        src2 = self.norm21(src2)        # torch.Size([1024, 1, 256])


        src12 = self.multihead_attn1(query=self.with_pos_embed(src1, pos_src1),        # torch.Size([256, 1, 256])#
                                   key=self.with_pos_embed(src2, pos_src2),         # torch.Size([1024, 1, 256])
                                   value=src2, attn_mask=src2_mask,         # torch.Size([1024, 1, 256])
                                   key_padding_mask=src2_key_padding_mask)[0]       # torch.Size([256, 1, 256])
        src22 = self.multihead_attn2(query=self.with_pos_embed(src2, pos_src2),
                                   ...
                                   key_padding_mask=src1_key_padding_mask)[0]       # torch.Size([1024, 1, 256])

        src1 = src1 + self.dropout12(src12)     # torch.Size([256, 1, 256])
        src1 = self.norm12(src1)        # torch.Size([256, 1, 256])
        src12 = self.linear12(self.dropout1(self.activation1(self.linear11(src1))))     # torch.Size([256, 1, 256])
        src1 = src1 + self.dropout13(src12)     # torch.Size([256, 1, 256])
        src1 = self.norm13(src1)    # torch.Size([256, 1, 256])

        src2 = src2 + self.dropout22(src22)     # torch.Size([1024, 1, 256])
        src2 = self.norm22(src2)        # torch.Size([1024, 1, 256])
        src22 = self.linear22(self.dropout2(self.activation2(self.linear21(src2))))     # torch.Size([1024, 1, 256])
        src2 = src2 + self.dropout23(src22)     # torch.Size([1024, 1, 256])
        src2 = self.norm23(src2)        # torch.Size([1024, 1, 256])
