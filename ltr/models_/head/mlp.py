...
class Mlp(nn.Module):
    ...
    def forward(self, x):       # x.shape:torch.Size([8, 196, 384])
        for i, linear in enumerate(self.layers):
            x = linear(x)       # torch.Size([8, 196, 384])
            if i != len(self.layers) - 1:
                x = self.act(x)     # torch.Size([8, 196, 384])
            x = self.drop(x)        # torch.Size([8, 196, 1])
        return x
...
class MlpHead(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        cls = self.cls_mlp(x)       # shape:torch.Size([8, 196, 1])
        reg = self.reg_mlp(x).sigmoid()     # shape:torch.Size([8, 196, 4])
