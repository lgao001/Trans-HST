...
class ConcatenatedFusion(nn.Module):
    ...
    def forward(self, z, x, z_pos, x_pos):
        ...
        # shape:torch.Size([8, 256*5, 1024])    template
        # shape:torch.Size([8, 1024*5, 1024])   search

        concatenated = torch.cat((z, x), dim=1)     # torch.Size([8, 260, 384])

        # concatenated = torch.cat((z, x), dim=2)
    ...
    #     # concatenated = torch.cat((z, x), dim=1)     # torch.Size([1, 260, 384])
    ...
    #         x_q_pos, x_k_pos = self.x_untied_pos_enc()
    #         # attn_pos_enc = (torch.cat((z_q_pos, x_q_pos), dim=1) @ torch.cat((z_k_pos, x_k_pos), dim=1).transpose(-2, -1)).unsqueeze(0)
    #         attn_pos_enc = (x_q_pos) @ (x_k_pos).transpose(-2, -1).unsqueeze(0)
