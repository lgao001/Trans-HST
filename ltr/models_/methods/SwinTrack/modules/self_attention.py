    ...
    def forward(self, x, q_ape, k_ape, attn_pos):
        '''
            ...
            Returns:
                torch.Tensor: (B, L, C)
        '''
        B, N, C = x.shape       # torch.Size([1, 260, 384])
        ...
        attn = q @ k.transpose(-2, -1)      # torch.Size([1, 8, 245, 245])
        attn = attn * self.scale            # torch.Size([1, 8, 245, 245)
        if attn_pos is not None:
            attn = attn + attn_pos          # attn_pos.shape:torch.Size([1, 8, 245, 245])
